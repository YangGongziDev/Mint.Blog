...
import { breakpointsTailwind, useBreakpoints, useEventListener, useMediaQuery, useTitle } from '@vueuse/core';
...
export const useAppStore = defineStore(SetupStoreId.App, () => {
  ...
  const isSmallWidth = breakpoints.smaller('md');
  const isSmallHeight = useMediaQuery('(max-height: 500px)');
  /** Is mobile layout */
  const isMobile = computed(() => isSmallWidth.value || isSmallHeight.value);
  ...
  scope.run(() => {
    ...
    watch(
      ...
      newValue => {
        ...
        const isFrontdeskLayout =
          currentRoute.meta.layout === 'frontdesk' || currentRoute.path.startsWith('/blog/surfer');

        if (isFrontdeskLayout) {
          if (newValue) {
            setSiderCollapse(true);
          } else {
            themeStore.applyThemeLayoutByRoute(router.currentRoute.value);
          }
          return;
        }
        ...
        if (newValue) {
          ...
          themeStore.setTemporaryThemeLayout('vertical');
          setSiderCollapse(true);
        } else {
          ...
          if (backup) {
            nextTick(() => {
              themeStore.setTemporaryThemeLayout(backup.layout);
