...
<template>
  <main class="gallery-page mx-auto max-w-screen-2xl px-4 py-0 md:px-6" :class="pageClass">
    ...
    <section class="image-panel"><div class="toolbar"><div><h2>{{ activeCategoryName }}</h2><p>共 {{ filteredImages.length }} 张图片</p></div></div>
      ...
      <div v-else class="image-grid"><article v-for="image in filteredImages" :key="image.id" class="image-card"><div class="image-cover" role="button" tabindex="0" @click="openPreview(image)" @keydown.enter="openPreview(image)"><img :src="image.url" :alt="image.name" loading="lazy" /></div><div class="image-info"><h3>{{ image.name }}</h3><div class="meta-row"><span>{{ image.categoryName }}</span><span>{{ image.resolution }}</span><span>{{ formatRatio(image.ratio) }}</span><span class="meta-actions"><span>{{ image.size }} MB</span><a class="download-action" :href="image.url" target="_blank" rel="noopener noreferrer" :download="image.name" @click.stop><DownloadOutlined /> 下载</a></span></div><p><CalendarOutlined /> {{ image.time || '未设置时间' }}</p></div></article></div>
    </section>
...
.gallery-page.dark .filter-panel, .gallery-page.dark .image-panel { border-color:rgb(51 65 85 / 90%); background:rgb(44 51 62 / 74%); box-shadow:0 8px 28px rgb(0 0 0 / 18%); }
