<template>
    ...
    <div v-if="titles && titles.length > 0" 
        ...
        <!-- 目录标题 -->
        <h2 class="theme-bg-secondary theme-text-primary flex items-center mb-2 font-bold text-gray-900 uppercase dark:text-gray-400">
            文章目录
        </h2>
...
// 初始化标题数据
function initTocData(container: Element | null): void {
    if (!container) return
    
    // 只提取二级、三级标题
    const levels: string[] = ['h2', 'h3']
    ...
    headings.forEach((heading: Element) => {
        ...
        // 标题等级， h2 -> 级别 2 ； h3 -> 级别3
        const headingLevel = parseInt(htmlElement.tagName.substring(1))
        // 标题文字
        const headingText = htmlElement.innerText
        // 标题的位置（距离顶部的距离）
        // 原：const offsetTop = htmlElement.offsetTop - 95
        const offsetTop = calcOffsetTopRelativeToScroller(htmlElement, 95)
